...
| C-11 Reproducibility | Pinned runtime/dev locks, digest-pinned image, CI, security suite, and benchmark | lock files, `tests/test_reproducibility.py`, GitHub Actions artifacts, and benchmark JSON | Replicate the locked workflow in an independent environment |
| C-12 Supply-chain hygiene | Dependency audit, Bandit, Trivy workflow, Dependabot, machine-readable triage policy, and release-review gate | `.github/workflows/security.yml` artifacts, `security/scan-policy.json`, `security/release-review.json`, and validator tests/docs | Complete the approved release review against actual scan artifacts for each release |

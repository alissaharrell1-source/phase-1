...
The first M1 deliverable is now present as `madva-deployment-validate`. It fails closed, emits redacted machine-readable results, and is runnable by CI or an operator without embedding credentials in the repository. Its `--live` mode checks staging OIDC discovery/JWKS, Vault health and optional credential access, OTLP connectivity, and audit-volume writability without printing secrets. The redacted [staging configuration record](STAGING_CONFIGURATION_RECORD.md) defines the deployment handoff fields and sign-off owners. The repeatable operator procedure is documented in the [staging validation runbook](STAGING_VALIDATION_RUNBOOK.md). The next M1 increment is populating that record for the selected staging services, running the validator, and attaching the redacted result to the release record. The M2 execution procedure is documented in the [staging disruption runbook](STAGING_DISRUPTION_RUNBOOK.md).

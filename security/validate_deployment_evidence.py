...
SCHEMA_PATH = Path(__file__).with_name("deployment-validation-evidence.schema.json")
...
def validate_evidence(data: object) -> list[str]:
    ...
    if isinstance(result, dict) and result.get("valid") is True and result.get("errors"):
        errors.append("valid_result_contains_errors")
    return errors


def main() -> int:
    parser = argparse.ArgumentParser(description="Validate MADVA deployment-validation evidence")
